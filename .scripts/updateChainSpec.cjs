...
if (!sourceFileName || !sourceFileName.endsWith('.json')) {
  console.error(
    'Please provide a chain spec file name (with .json extension) to download from the zenchain-node repo at https://raw.githubusercontent.com/zenchain-protocol/zenchain-node/main/chain-specs/FILENAME'
  );
  process.exit(1);
}
